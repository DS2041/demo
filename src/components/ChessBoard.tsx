...
const ChessBoard: React.FC = () => {
  ...
  const getIllegalMoveMessage = (source: Square, target: Square) => {
  ...
  useEffect(() => {
    ...
    const name =
      currentTurn === "white"
        ...
        ? myName
        : opponentName;
    const message = thinkingDialogues[
      Math.floor(Math.random() * thinkingDialogues.length)
    ].replace(
      "{player}",
  ...
  // Timer start effect for creator
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    socket.on("opponent-move", (move: PlayerMove) => {
      ...
      if (newGame.isCheckmate()) {
        ...
        const winnerMessage =
          playerColor === winnerColor ? "You win!" : "You lose!";
    ...
    socket.on("timer-update", ({ white, black, active }: TimerUpdate) => {
      setWhiteTime(white);
      setBlackTime(black);
    ...
    socket.on("end-room", (data: { winner?: string; reason?: string }) => {
      setTimerRunning(false);
      setGameResult({
        winner: (data?.winner as "white" | "black") || null,
        reason: data?.reason || "Room ended",
      });
  ...
  for (let i = 0; i < 8; ++i) {
    for (let j = 0; j < 8; ++j) {
      ...
      const squareVal = `${file}${rank}` as Square;
  ...
  const onDrop = (sourceSquare: Square, targetSquare: Square) => {
    ...
    const legalMove = possibleMoves.find((m) => m.to === targetSquare);

    if (!legalMove) {
      const message = getIllegalMoveMessage(sourceSquare, targetSquare);
      toast.error(message, {
        style: {
          background: "#222",
          color: "#fff",
  ...
  useEffect(() => {
    if (gameResult) {
      const isLoser =
        gameResult.winner !== null && playerColor !== gameResult.winner;
      if (isLoser) {
        ...
        <div className="flex flex-col items-center gap-4 w-full text-xs text-gray-400 uppercase tracking-wider">
            ...
            <div className="grid grid-cols-3 items-center w-full">
              <div className="flex items-center gap-2 min-w-0 relative">
                ...
                <span
                  ...
                  title={playerColor === "white" ? myName : opponentName}
                >
                  {playerColor === "white" ? myName : opponentName}
                </span>
              </div>

              <span className="justify-self-center font-extrabold text-base text-gray-300 px-2">
                VS
              </span>
              ...
              <div className="flex items-center gap-2 min-w-0 justify-end relative">
                ...
                <span
                  className="font-bold text-lg truncate text-white"
                  title={playerColor === "white" ? opponentName : myName}
                >
          ...
          <div className="flex justify-between w-full z-10 relative items-center gap-2">
            <div className="flex items-center gap-3">
              {getCurrentTurn() === "white" && (
                <span
                  className="w-3 h-3 rounded-full bg-emerald-400 animate-pulse ring-2 ring-emerald-500"
                  title="White's turn"
                />
              )}
            ...
            <div
              className="flex items-center gap-2 bg-brand/20 px-4 py-1 rounded-lg border border-brand/70 shadow-md text-brand font-bold text-base md:text-lg mx-2 select-none"
              title="Room/Game Name"
            >
              <TrophyIcon className="w-7 h-7 text-yellow-400 drop-shadow" />
              <span className="text-center truncate max-w-xs text-white">
                {roomName}
              </span>
            </div>
          ...
          {gameResult && (
            <div className="mt-2 text-base text-amber-400 font-bold bg-black/50 rounded p-2 text-center max-w-md">
              ...
              {gameResult.winner &&
                (() => {
                  const winnerName =
                    gameResult.winner === "white" ? myName : opponentName;
                  const isWinner = playerColor === gameResult.winner;
                  return (
                    <>
                      | Winner:{" "}
                      <span className="text-emerald-400">{winnerName}</span>
          ...
          {/* Chessboard */}
          <div
            className="rounded-xl overflow-hidden border-4 border-brand/60 shadow-2xl backdrop-blur-xl bg-white/10"
            style={{
          ...
          <div className="mt-4 w-full max-w-md bg-black/30 rounded p-2 text-xs text-gray-300 backdrop-blur-md border border-brand/30 max-h-32 overflow-y-auto">
            <h3 className="font-semibold text-brand mb-1 text-sm">
              Move History
            </h3>
            {moveHistory.length === 0 && (
              <div className="text-gray-200">No moves yet</div>
            )}
